...
public class FederatedAuthenticator extends AbstractApplicationAuthenticator
    ...
    protected void initiateAuthenticationRequest(HttpServletRequest request, HttpServletResponse response, AuthenticationContext context) throws AuthenticationFailedException {
        ...
        log.debug("sessionKey :"+sessionKey +" ~ operator : "+operator+" ~ acrValue : "+acrValue+" ~ federatedAouthEndpoint : "+federatedAouthEndpoint+" ~ federatedCallBackUrl : "+federatedCallBackUrl+" ~ client_id : "+clientId+" ~ nonce : "+nonce);
        if(paramMap.containsKey(LOGIN_HINT)){
            String loginHint = paramMap.get(LOGIN_HINT);
            federatedMobileConnectCallUrl = federatedAouthEndpoint +"?response_type=code&state="+sessionKey+"&nonce="+nonce+"&max_age=3600&scope=openid&response_type=code&redirect_uri="+federatedCallBackUrl+"&client_id="+clientId+"&acr_values="+acrValue+"&login_hint="+loginHint;
        }else{
            federatedMobileConnectCallUrl = federatedAouthEndpoint +"?response_type=code&state="+sessionKey+"&nonce="+nonce+"&max_age=3600&scope=openid&response_type=code&redirect_uri="+federatedCallBackUrl+"&client_id="+clientId+"&acr_values="+acrValue;
        }
    ...
    protected void processAuthenticationResponse(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, AuthenticationContext authenticationContext) throws AuthenticationFailedException {
        ...
        try {
            new UserProfileManager().createUserProfileLoa2(authenticationContext.getProperty(Constants.MSISDN).toString(), operator, Constants.OPENID);
        } catch (UserRegistrationAdminServiceIdentityException e) {
            log.error("Error Creating User Profile User Registration Admin Service : " + e.getMessage());
    ...
    private String generateRandomNumber(String countryCode){
        ...
        return countryCode + String.valueOf(modulesValue);
