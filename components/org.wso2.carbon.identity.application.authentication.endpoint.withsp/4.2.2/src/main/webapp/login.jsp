...
<fmt:bundle basename="org.wso2.carbon.identity.application.authentication.endpoint.i18n.Resources">
    ...
    <input type="hidden" id="baseURLWithPort" value='<%=request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort() %>'>
				...
				if (CharacterEncoder.getSafeText(request.getParameter(Constants.AUTH_FAILURE_MSG)) != null) {
					...
					if (errorMessage.equalsIgnoreCase("login.fail.message")) {
						if (request.getParameter("acr_values") != null) {
							...
							URL url = new URL(baseURL + "/UserRegistration-1.0-SNAPSHOT/webresources/endpoint/user/authenticate/add?scope=" + scope + "&redirecturi=" + redirectUri + "&clientid=" + clientId + "&acrvalue=" + arc + "&responsetype=" + responseType + "&operator=" + operator + "&msisdn=" + msisdn+"&nonce="+nonce+"&state="+state);
							...
							if (conn.getResponseCode() != 200) {
								...
								token = br.readLine();
								conn.disconnect();
								String site = new String("/dashboard/register.jag?token=" + token + "&operator=" + operator +"&acr="+arc);
	...
	<header class="site-header">
		  <div class="site-header__inner site__wrap">
			...
			<a href="/dashboard/selfcare/index.html"><img src="images/svg/mobile-connect.svg" alt="Mobile Connect&nbsp;Logo" width="150" class="site-header__logo"></a>
			<% if(!operator.isEmpty()){ 
